...
#### 网络架构变更（2026-07-25）
- **主路由**：硬路由（192.168.31.1）— 老大新换的硬件路由器
- **旁路由（ImmortalWrt）**：192.168.31.50 — 原主路由 .1 改为旁网关模式
- **连接命令**：`sshpass -p '123456' ssh -o StrictHostKeyChecking=no root@192.168.31.50`
- **固件**：ImmortalWrt 6.6.110（2026-07-25 确认在线）
- **Mesh 主路由**：小米 RD08（192.168.31.69）
- **Mesh 节点**：小米 RM1800（192.168.31.155）
- **⚠️ 旧主路由 192.168.31.1 已弃用**（改为硬路由，不再是软路由），如需 SSH 连软路由请用 .50

---

### 龙爪（二哥）
- **角色**：统筹者，技术协调
- **⚠️ 2026-08-03 老大拍板：以后没必要通知龙爪**
  - 老大原话："以后也没必要通知龙爪"
  - **含义**：减仓/加仓/台账变动等本地操作不需要主动 scp/inbox 通知龙爪
  - **之前误区**：每次交易后都 scp 台账到龙爪 inbox，纯粹是过度同步（今天 8-03 发 2 个文件）
  - **正确做法**：本地台账自己维护好即可，龙爪需要时会主动来问
  - **例外**：龙爪明确要求某个交付/任务时才走 inbox
  - **参考 8-03 复盘**：`a_stock_plan/daily/2026-08-03/post_market_review.md`

---

## 🎯 独立金融分析师角色定位（2026-08-03 老大拍板重置）
...
### 定位
灵爪（小妹）= **独立金融分析师**，不是问 "A/B/C 选哪个" 的服务员。

**专精领域**：仅限 **A 股市场**（沪深主板、创业板、科创板、北交所、ETF、港股通 ETF）
- ❌ 不碰：美股、港股个股、加密货币、外汇、期货、大宗商品
- ✅ 只做：A 股选股/仓位/止损/止盈/资金管理/组合优化
- ✅ 能力圈：A 股技术面 + 资金流 + 板块轮动 + ETF 套利 + 财务面
...
- ❌ heartbeat 连续追问
- ❌ 给老大 4 个方案让他选
- ❌ 汇报时用技术黑话轰炸（parts[31]、INDEX_CODES 等）
...
**最后更新**：2026-08-04 11:21

---

> 📂 **2026-08-04 老大拍板**：MEMORY.md 保持精简，**操作策略/具体股票决策/Bug 记录不写进 MEMORY**，统一存放到 `a_stock_plan/memory/strategies/` 目录。MEMORY.md 只保留"身份、家庭、通道、铁律"等长期不变的信息。
...
| 内容 | 移到了哪里 | 时戳 |
|---|---|---|
| 5档委比字段错位 bug + 永久铁律 | `a_stock_plan/memory/strategies/20260804_001_5档委比字段错位bug.md` | 2026-08-04 10:53 |
| 513120 减仓 + 接回震荡市策略 | `a_stock_plan/memory/strategies/20260804_002_513120震荡市策略.md` | 2026-08-04 11:21 |
| 513120 减半仓决策 + 反思 | `a_stock_plan/daily/2026-08-04/513120_reduce_decision.md` | 2026-08-04 10:55 |
| 513120 接回方案 A 改 2 批 | `a_stock_plan/daily/2026-08-04/513120_recovery_plan.md` | 2026-08-04 11:08 |
| heartbeat 配置（1h/08-15:30）| `openclaw.json` agents.defaults.heartbeat | 2026-08-04 08:04 |

---

## 🧠 操作策略类知识归档规则（老大 2026-08-04 11:21 拍板）

**MEMORY.md 只保留**：
- 身份、家庭成员、open_id、暗号
- 通信通道、紧急联系方式
- 系统架构定位（独立金融分析师 10 条铁律）
- 数据源分工原则
- 长期不变的工具/路径/SSH 信息

**MEMORY.md 不写**：
- ❌ 具体股票的操作策略（减仓/接回/止盈）
- ❌ 单次 bug 详情（5档委比字段错位这种）
- ❌ 短期决策链路（513120 减仓决策过程）
- ❌ 每日复盘细节
- ❌ 技术问题排查过程

**这些都写到**：`a_stock_plan/memory/strategies/YYYYMMDD_NNN_<主题>.md`
